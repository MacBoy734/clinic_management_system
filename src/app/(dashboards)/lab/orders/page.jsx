...
// PharmacyOrdersTab — lets the lab tech order consumable supplies (reagents,
// swabs, collection tubes, etc.) from the in-house pharmacy for the lab.
//
// APIs:
//   GET  /api/lab/orders        → { orders: [...] } — lab's existing orders
//   POST /api/pharmacy/orders   → create new order
//        Body: { department: 'lab', requested_by: string, items: [{name, quantity, notes}] }
...
export default function PharmacyOrdersTab() {
  ...
  const requestedBy = user?.name || 'Lab Tech'
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <StatCard icon="checkCircle" color="green" label="Fulfilled" value={fulfilledCount} sublabel="delivered by pharmacy" />
      ...
      {!filtered.length ? (
        <EmptyState
          ...
          description={filter === 'all'
            ? 'Order lab supplies (reagents, swabs, collection tubes, etc.) from the in-house pharmacy using the “New Order” button.'
      ...
      >
        <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700/60 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">
              Order lab supplies · requested by {requestedBy}
            </p>
          </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-2">
                  <div className="sm:col-span-6">
                    ...
                    <input
                      ...
                      placeholder="e.g. EDTA Collection Tubes"
                      className={inputCls}
                    />
                  </div>
